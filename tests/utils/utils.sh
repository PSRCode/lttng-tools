...
function start_lttng_sessiond_opt()
{
	...
	if [ -z $(pgrep --full lt-$SESSIOND_BIN[^\[]) ]; then
		# Have a load path ?
		if [ -n "$load_path" ]; then
			$DIR/$SESSIOND_PATH_REL/$SESSIOND_BIN --load "$1" --background --consumerd32-path="$DIR/$CONSUMERD_PATH/$CONSUMERD_BIN" --consumerd64-path="$DIR/$CONSUMERD_PATH/$CONSUMERD_BIN"
		else
...
function create_lttng_session ()
{
	...
	$TESTDIR/$LTTNG_PATH_REL/$LTTNG_BIN create $sess_name $trace_path $opt > $OUTPUT_DEST
	ret=$?
	if [[ $expected_to_fail -eq "1" ]]; then
		test "$ret" -ne "0"

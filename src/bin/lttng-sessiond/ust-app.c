		...
		struct ust_app_event *ua_event)
{
	...
	DBG("🥓: ustctl_counter_create_event: sock=%d, map_fd=%d",
			app->sock, ua_map->obj->handle);
	/* Create UST event on tracer */
	pthread_mutex_lock(&app->sock_lock);
		...
		struct ust_app_event_notifier_rule *ua_event_notifier_rule)
{
	...
	condition = lttng_trigger_get_condition(ua_event_notifier_rule->trigger);
	assert(condition);
	assert(lttng_condition_get_type(condition) == LTTNG_CONDITION_TYPE_ON_EVENT);
		...
		struct ust_app_map *ua_map)
{
	...
	ret = create_buffer_reg_map(buffer_reg_uid->registry, ua_map, &buffer_reg_map);
	if (ret < 0) {
	...
	ret = create_map_object(usess, ua_sess, ua_map,
			buffer_reg_uid->registry->reg.ust);
	assert(ret == 0);
	if (ret < 0) {
	 ...
	 * Setup the streams and add it to the session registry.
	 */
	ret = setup_buffer_reg_map(buffer_reg_uid->registry, ua_map, buffer_reg_map, app);
	if (ret < 0) {
		...
		struct lttng_map_key_value_pair_list *kv_pair_list)
{
	...
	bitness = lttng_map_get_bitness(umap->map) == LTTNG_MAP_BITNESS_32BITS ? 32 : 64;

	buf_reg_uid = buffer_reg_uid_find(usess->id, bitness, usess->uid);
	if (!buf_reg_uid) {
		/*
		...
		int loglevel_value, char *model_emf_uri, uint64_t tracer_token)
{
	 ...
	 * return a negative error or else the application will get closed.
	 */
	ret = ustctl_reply_register_event(sock, counter_index, counter_index, ret_code);
	if (ret < 0) {
		if (ret != -EPIPE && ret != -LTTNG_UST_ERR_EXITING) {
			ERR("UST app reply event failed with ret %d", ret);
		} else {

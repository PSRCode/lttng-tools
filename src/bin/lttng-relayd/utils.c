...
/*
 * Provide support for --group-output-by-session for producer < 2.11.
 * Take the stream path, extract all available information, craft a new path to
 * the best of our ability enforcing the group by session name.
 *
 * Return the allocated string containing the new stream path or else NULL.
 */
char *backward_compat_stream_group_by_session_name(const char *path, const char
		*local_session_name) {
	int ret;

...
static enum lttng_error_code lttng_event_rule_kernel_tracepoint_mi(
		const struct lttng_event_rule *rule, struct mi_writer *writer)
{
	...
	status = lttng_event_rule_kernel_tracepoint_get_filter(rule, &filter);
	assert(status == LTTNG_EVENT_RULE_STATUS_OK || status == LTTNG_EVENT_RULE_STATUS_UNSET);

	/* Open event_rule_kernel_tracepoint. */
	...
	/* Close event_rule_kernel_tracepoint. */
...
struct lttng_event_rule *lttng_event_rule_kernel_tracepoint_create(void)
{
	...
	tp_rule->parent.mi = lttng_event_rule_kernel_tracepoint_mi;

...
static enum lttng_error_code lttng_event_rule_kernel_syscall_mi(
		const struct lttng_event_rule *rule, struct mi_writer *writer)
{
	...
	/* Open event_rule_kernel_syscall. */
	...
	/* Close event_rule_kernel_syscall. */
				...
				emission_site)
{
	...
	syscall_rule->parent.mi = lttng_event_rule_kernel_syscall_mi;

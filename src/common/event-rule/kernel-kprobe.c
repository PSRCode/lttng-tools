...
static enum lttng_error_code lttng_event_rule_kernel_kprobe_mi(
		const struct lttng_event_rule *rule, struct mi_writer *writer)
{
	...
	/* Probe location.*/
	ret = lttng_kernel_probe_location_mi(location, writer);
	if (ret) {
		goto mi_error;
	}

	/* Close event_rule_kernel_kprobe. */
		...
		const struct lttng_kernel_probe_location *location)
{
	...
	krule->parent.mi = lttng_event_rule_kernel_kprobe_mi;

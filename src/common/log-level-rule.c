...
LTTNG_HIDDEN
enum lttng_error_code lttng_log_level_rule_mi(
		const struct lttng_log_level_rule *rule, struct mi_writer *writer)
{
	int ret;
	...
	case LTTNG_LOG_LEVEL_RULE_TYPE_EXACTLY:
		type_str = mi_lttng_log_level_rule_type_str_exactly;
		status = lttng_log_level_rule_exactly_get_level(rule, &level);
		element_str = mi_lttng_element_log_level_rule_exactly;
		break;
	case LTTNG_LOG_LEVEL_RULE_TYPE_AT_LEAST_AS_SEVERE_AS:
		type_str = mi_lttng_log_level_rule_type_str_at_least_as_severe_as;

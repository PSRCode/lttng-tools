...
static
enum lttng_error_code lttng_event_expr_event_payload_field_mi(const struct lttng_event_expr *expression,
...
static
enum lttng_error_code lttng_event_expr_channel_context_field_mi(const struct lttng_event_expr *expression,
		struct mi_writer *writer)
{
	int ret;
	enum lttng_error_code ret_code;
...
static
enum lttng_error_code lttng_event_expr_app_specific_context_field_mi(const struct lttng_event_expr *expression,
		struct mi_writer *writer)
{
	int ret;
	enum lttng_error_code ret_code;
	...
	assert(expression);
	assert(writer);
	assert(expression->type == LTTNG_EVENT_EXPR_TYPE_APP_SPECIFIC_CONTEXT_FIELD);
...
static
enum lttng_error_code lttng_event_expr_array_field_element_mi(const struct lttng_event_expr *expression,
		struct mi_writer *writer)
{
	int ret;
	enum lttng_error_code ret_code;
...
LTTNG_HIDDEN
enum lttng_error_code lttng_event_expr_mi(const struct lttng_event_expr *expression,
		struct mi_writer *writer)
{
	int ret;
	enum lttng_error_code ret_code;
	...
	/* Type */
	ret = mi_lttng_writer_write_element_string(writer, config_element_type, mi_lttng_event_expr_type_string(expression->type));
	if(ret) {
		goto mi_error;
	}
	...
	case LTTNG_EVENT_EXPR_TYPE_EVENT_PAYLOAD_FIELD:
		ret_code = lttng_event_expr_event_payload_field_mi(expression, writer);
		break;
	case LTTNG_EVENT_EXPR_TYPE_CHANNEL_CONTEXT_FIELD:
		ret_code = lttng_event_expr_channel_context_field_mi(expression, writer);
		break;
	case LTTNG_EVENT_EXPR_TYPE_APP_SPECIFIC_CONTEXT_FIELD:
		ret_code = lttng_event_expr_app_specific_context_field_mi(expression, writer);
		break;
	case LTTNG_EVENT_EXPR_TYPE_ARRAY_FIELD_ELEMENT:
		ret_code = lttng_event_expr_array_field_element_mi(expression, writer);
		break;
	default:
		abort();
	}

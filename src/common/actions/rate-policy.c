...
typedef enum lttng_error_code (*rate_policy_mi_cb)(
...
static enum lttng_error_code lttng_rate_policy_every_n_mi(
...
static enum lttng_error_code lttng_rate_policy_once_after_n_mi(
		const struct lttng_rate_policy *rate_policy, struct mi_writer *writer)
{
	int ret;
	enum lttng_error_code ret_code;
		...
		uint64_t threshold)
{
	...
	lttng_rate_policy_init(&policy->parent,
			...
			lttng_rate_policy_once_after_n_mi);
...
LTTNG_HIDDEN
enum lttng_error_code lttng_rate_policy_mi(
		const struct lttng_rate_policy *rate_policy, struct mi_writer *writer)
{
	int ret;
	enum lttng_error_code ret_code;
	...
	assert(rate_policy);
	assert(writer);
	assert(rate_policy->mi);

	/* Open rate_policy element. */
	ret = mi_lttng_writer_open_element(
	...
	ret = mi_lttng_writer_write_element_string(
			writer, config_element_type, mi_lttng_rate_policy_type_string(rate_policy->type));
	if (ret) {
		goto mi_error;
	}
